...
final class Store: ObservableObject {
    ...
    private let fileName = "tinnitustrack_entries.json"
    ...
    private func seed() {
        entries = [
            Entry(date: Calendar.current.date(byAdding: .day, value: -1, to: Date())!, rating: 3, note: "", tags: []),\nEntry(date: Calendar.current.date(byAdding: .day, value: -2, to: Date())!, rating: 4, note: "", tags: []),\nEntry(date: Calendar.current.date(byAdding: .day, value: -3, to: Date())!, rating: 5, note: "", tags: [])
        ]
    }
    private func load() {
